...
client.login("MTI5MjgzNDg5Mzk5ODMyOTg5Ng.G73Hv_.JPdkaVFOMJMG98LmdPz-0sx9sz2Ka-ojBzY6iQ");
...
client.on("interactionCreate", async (interaction) => {
  if(interaction.customId == 'how'){
     interaction.reply({content: `** يرجي اتباع الخطوات الذي بالشرح :**
|| || `, ephemeral: true})
  }
});
...
client.on(`interactionCreate` ,async interaction => {
  ...
  if (interaction.customId == 'BuyModal'){
    ...
    let amount = Count
    let count = 0
    if (!guild) return interaction.reply({ content: `** Sorry, I couldn't find the server that I entered, please enter the server through the button, check the server from above and try again..

 عذرا , لم اتمكن من العثور علي الخادم الذي  ادخلت ايديه رجاءً ادخلني السيرفر من عبر زر اضافة البوت من فوق و اعد المحاولة..**` }).catch(() => { interaction.channel.send({ content: `** Sorry, I couldn't find the server that I entered, please enter the server through the button, Add Bot to server from above and try again..

 عذرا , لم اتمكن من العثور علي الخادم الذي  ادخلت ايديه رجاءً ادخلني السيرفر من عبر زر اضافة البوت من فوق و اعد المحاولة..**` }) });
    if (amount > alld.length) return interaction.reply({ content: `**<: You Can't Buy this Quantity, I don't have this Number of Mebers to know what Quantity you Can buy, Please Write $stock

 لا يمكنك شراء هذه الكمية، لا يتوفر لدي هاذا العدد من الاعضاء لمعرفة الكمية التي يمكنك شرائها، يرجي كتابة stock$**` }).catch(() => { interaction.channel.send({ content: `** You Can't Buy this Quantity, I don't have this Number of Mebers to know what Quantity you Can buy, Please Write $stock
...
يمكنك شراء هذه الكمية، لا يتوفر لدي هاذا العدد من الاعضاء لمعرفة الكمية التي يمكنك شرائها، يرجي كتابة $stock**` }) });;
    const tra = new MessageEmbed()
            ...
            .setTitle('عملية شراء اعضاء')
              .setDescription(`**لأكمال عملية شراء الاعضاء , يرجي نسخ الكود بالأسفل و إتمام عملية التحويل.
...
جاري ادخال الاعضاء يرجي الانتظار..**` })
    ...
    .setDescription(`\`\`\`💫 - عش تجربة مميزة مع Pluto Services  : discord.gg/pl-s.\`\`\``)
          if (Log){
...
client.on('messageCreate', async (message) => {
  ...
  if (isNaN(quantity)) {
    return message.reply('** اكتب العدد الذي تريدة**');
  }
